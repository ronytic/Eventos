...
<section class="">
    <div class="container">
        ...
        <?php
        $usuario=new usuario;
        $usu=$usuario->mostrarTodoRegistro("nivel!=1",1,"paterno,materno,nombre");
        ?>
        <div class="row">
             <div class="col-sm-6">
                <h3 class="section-title">Agregar Personal al Evento</h3>
                Personal:
                <select class="form-control" name="codusuarios">
                    <?php foreach($usu as $u){?>
                    <option value="<?php echo $u['codusuarios']?>"><?php echo $u['paterno']?> <?php echo $u['materno']?> <?php echo $u['nombre']?>, Cargo: <?php echo $u['cargo']?></option>
                    <?php }?>
                </select>
                <a href="" class="btn btn-info" id="agregar">Agregar</a>
             </div>
             <div class="col-sm-6">
                <h3 class="section-title">Personal a Cargo del Evento</h3>
                <div class="mostrarpersonal"></div>
             </div>
        </div>
    </div>
</section>
<script language="javascript" type="text/javascript">
var codreserva=<?php echo $codreserva?>;
$(document).on("ready",function(){
    
    $("#agregar").click(function(e) {
        e.preventDefault();
        var codusuarios=$("[name=codusuarios]").val();
        $.post("agregarpersonal.php",{"codusuarios":codusuarios,"codreserva":codreserva},function(data){
            listar();
        });
    });
    $(document).on("click",".eliminar",function(e){
        ...
        if(confirm("¿Esta seguro que desea Eliminar este Evento?")){
            ...
            $.post("eliminarpersonal.php",{'codpersonal':cod},function(data){
                listar();
            });
        }
    })
...
function listar(){
    $.post("mostrarpersonal.php",{"codreserva":codreserva},function(data){
         $(".mostrarpersonal").html(data)   
    });    
}
</script>
